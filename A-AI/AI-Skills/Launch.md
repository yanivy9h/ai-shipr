...
Ask which product this launch covers before reading any files. Use `[product-name]/` as the path prefix for all file reads below (e.g., `patient-prep/I-Initiatives/`, `patient-prep/H-Hypotheses/`).

**If `product_mode: single` (or not set):**
Read from root-level folders.

Ask for the initiative name if not provided. Then read:
- The relevant initiative file from `I-Initiatives/` (or `[product-name]/I-Initiatives/` in multi mode)
- The linked hypothesis file from `H-Hypotheses/` (or `[product-name]/H-Hypotheses/` in multi mode)
