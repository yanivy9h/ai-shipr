...
Ask: "Are you planning for one product or across the full portfolio?" Then:
- Read `shared/Portfolio-Roadmap.md` — for cross-product overview and PM time split
- Read `shared/Constraints.md` — for shared capacity and budget constraints
- For each product listed under `products:` in Settings.md (or the selected product):
  - `[product]/S-Strategy/Strategic-Bets.md`
  - `[product]/S-Strategy/KPIs.md`
  - `[product]/S-Strategy/Constraints.md`
  - All files in `[product]/I-Initiatives/`
  - Status fields from all files in `[product]/H-Hypotheses/`
- Label all output sections by product name. Include a cross-product sequencing section when planning across the full portfolio.
